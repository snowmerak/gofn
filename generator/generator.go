...
func GenerateFor(outDir string, structs []parser.StructInfo, funcs []parser.FuncInfo) error {
	...
	if len(structs) > 0 {
		...
		for _, s := range structs {
			...
			// create an immutable constructor
		...
		out := filepath.Join(outDir, pkg+"_structs_gen.go")
	...
	if len(funcs) > 0 {
		...
		for _, f := range funcs {
			...
			// generate a curried wrapper if single result
			if len(f.Results) <= 1 {
				wrapper := generateCurriedFunc(f)
				buf.WriteString(wrapper + "\n")
			}
		}
		out := filepath.Join(outDir, pkg+"_funcs_gen.go")
		formatted, err := formatSource(buf.Bytes())
		if err != nil {
			return err
		}
